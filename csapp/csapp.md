...
- 实现带头尾字段的堆块的堆内存分配器
- 最开始尝试了无footer的chunk结构，这样的话会产生问题，不想从头遍历查找chunk（从时间上也不现实）只能使用指针来表示内存相邻的前一个块，显然不如加入一个footer
- 需要注意指针类型对指针运算的影响，通常直接转换为`char*`类型便于操作
- 书里给出来的freelist相关的函数和宏相当好用
- 写完之后看了一下网上的一些wp，发现只要是隐式链表的好像都差不多（毕竟简单
