...
- part C很 ~~难~~ 麻烦，需要解决流水线的数据冒险，同时考虑到各种优化问题，需要把第四章后半部分和第五章都搞定，这个等其他lab做完了再回来研究。

### part C

- 这部分看书看了好久，然而看进去的东西少得可怜，只是学习了一下各种优化方式，以及流水线这部分大概讲了什么。
- 可优化的点有：
  - 添加iaddq指令
  - 连续对同一个寄存器的mrmovq和rmmovq指令导致PIPE的加载/使用冒险
  - 循环展开
  - 流水线优化
    - 预测分支优化
    - 不必要气泡消除
...
- 不如shelllab一样给出一些辅助的函数，文件的读取（字符串处理）要自己实现，没有任何难度但是好烦人（
- 这里核心逻辑就是一个很简单的实现LRU的函数，对set * line的二维数组操作，加上block不需要考虑，整个模拟cache实在没有什么难度，读了一遍之后直接搬了网上的代码（实在没多少时间了=-=

### part B

- 针对三个固定大小的矩阵设计转置函数，要求尽可能降低miss率
- 给定的cache是32个set，E=1（直接相连），每个block支持32byte

1. 基本思路是将数组按8*8分块，以适应block的大小（每个block支持4个int大小的size）
...
2. 1中存在的问题在于，在`B[l][k] = A[k][l]`中，`B[l]`和`A[k]`并非同一行，因此会导致引用A的行时刷掉cache中的B行，所以取消（展开）原有的第四层循环，直球地用八个变量代替循环
...
3. 做到2就满分了（32\*32矩阵的miss在300以内），但是在知乎上看到了一个理论值的解法（miss = 256），是将第三层循环也展开进行转置，单独优化对角线所在分块的转置，这部分代码比较冗长就不放了，思考起来也比较麻烦

4. 对于64\*64的矩阵，不能直接沿用32\*32的分为4\*4组的策略（此时一行8字节，缓存block大小4字节，导致相同块被多次映射），首先想到的解决方法是和上面一样，直接分割成8\*8块。结果是miss次数非常多，改分为16\*16块，得到的也不是满分。满分的题解参考如下（我是写不出来了=-=）
...
5. 对于61*67的矩阵，沿用4中的分组策略，再对剩余的小块单独操作即可满分
...
- 带头尾字段的堆块，和CSAPP设计一致
- 最开始尝试了无footer的chunk结构，这样的话会产生问题，不想从头遍历查找chunk的话（时间上也不现实）只能使用指针来表示内存相邻的前一个块，显然不如加入一个footer
- 需要注意指针类型对指针运算的影响，通常直接转换为`char*`类型便于操作，在设计宏的时候踩了几次雷
- 书里给出来的freelist相关的函数和宏相当好用
- firstfit最后是52-56分之间，改成nextfiit后立刻来到了80+
- free后立即合并，合并时检查相邻chunk的alloc状态，进行不合并/合并前块/合并后块/合并前后块，合并后更新last_free标记，用于next-fit循环

```txt
Results for mm malloc:
trace  valid  util     ops      secs  Kops
 0       yes   86%    5694  0.000259 21959
 1       yes   91%    5848  0.000291 20103
 2       yes   89%    6648  0.000490 13570
 3       yes   91%    5380  0.000311 17293
 4       yes   66%   14400  0.000218 65995
 5       yes   86%    4800  0.001410  3404
 6       yes   83%    4800  0.001021  4699
 7       yes   55%   12000  0.000204 58795
 8       yes   51%   24000  0.001300 18456
 9       yes   26%   14401  0.075024   192
10       yes   47%   14401  0.002900  4966
Total          70%  112372  0.083429  1347
...
- 首先loader向内核申请一个空闲的进程结构体（PCB），内核会去初始化结构体（xv6里面是Env，Linux里是task_struct），结构体用于内核的进程管理。初始化结构体的过程中，内核需要为进程申请物理页并进行映射，同时设置进程的虚拟地址空间的内核部分。
- loader得到系统返回的结构体后，在磁盘上打开目标程序的文件，读ELF文件头和程序段头表，计算程序段的大小，根据大小申请物理内存并映射为虚拟内存，再将程序按内存页大小（CSAPP中称每一个这样的程序部分为chunk）读入到虚拟内存。然后为进程栈空间申请一个物理页并映射到虚拟内存（xv6）以初始化栈空间
- 之后加载程序使用的动态链接库，并对程序中引用的动态链接的符号进行重定位（根据重定位表查询符号表，得到符号的内存地址，修改重定位符号的内存值）
- 最后跳转到入口函数地址，调用_start执行该程序
...
- CPU通过虚拟内存前往MMU中对应PTE查找物理内存中的VA对应的VP，这个过程首先会检查PTE的有效位判断缓存命中
  - 如果有效位为0，即缓存不命中，则引发缺页异常，进入缺页异常处理，从物理内存中选择一个VP作为牺牲页
    - 如果牺牲页被修改过，则需要写回到磁盘的VP中
  - 异常处理程序将对应的磁盘中的VP写入到物理内存中，返回到引发异常的指令重新执行
  - 如果命中则直接访问物理内存的字节
